...
class Evaluator:
    ...
    def save_epoch(self, epoch):
        model_path = os.path.join('saved_models',self.conf['model_name'])
    ...
    def load_epoch(self, epoch):
        model_path = os.path.join('saved_models',self.conf['model_name'])
        assert os.path.exists(os.path.join(model_path,'weights_epoch_%d.h5' % epoch)), 'Weights at epoch %d not found' % epoch
        self.model.load_weights(os.path.join(model_path,'weights_epoch_%d.h5' % epoch))
    ...
    def train(self):
        ...
        #    def bad_answer_generator(self, n):
        #        yeild self.pada(random.sample(list(self.answers.values()), len(good_answers)))
        ...
        #    print('Fitting epoch %d' % i, file=sys.stderr)
            ...
            self.save_epoch(i)
            # implement some early stopping logic 
            if self.conf['early_stopping_patience'] > 0:
                if (val_loss['loss'] < hist.history['val_loss'][0]) & ((i - val_loss['epoch']) >= self.conf['early_stopping_patience']):
                    log("stopping early on epoch {0} with loss {1}".format(i, val_loss['loss']))
                    break
